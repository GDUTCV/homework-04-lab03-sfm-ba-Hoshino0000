...
def compute_ba_residuals(parameters: np.ndarray, intrinsics: np.ndarray, num_cameras: int, points2d: np.ndarray,
    ...
    homo_3d_points_T = np.transpose(np.concatenate((selected_points3d, np.ones((selected_points3d.shape[0], 1))), axis=1))
    selected_extrinsics = extrinsics[camera_idxs]
    points2d = np.einsum('ijk,ki->ij', np.matmul(intrinsics, selected_extrinsics), homo_3d_points_T)
    points2d /= points2d[:, -1].reshape((points2d.shape[0], 1))
    points2d = points2d[:, :-1]
    residuals = np.linalg.norm(points2d - points2d, axis=1)
